  ...
  if (MPI_FOUND)
    ...
    foreach (lang CXX C Fortran)
      ...
      if (TARGET "MPI::MPI_${lang}")
        list(APPEND mpi_TARGETS "MPI::MPI_${lang}")
      endif()
    endforeach()
...
function (libfind_find_openmp)
  ...
  if (OpenMP_C_FLAGS)
    ...
    endforeach()
    set(openmp_TARGETS ${openmp_TARGETS} PARENT_SCOPE)
  else ()
...
function (libfind_pkgconfig PKG)
  ...
  if (${PKG}_I_FOUND)
    ...
    foreach (lib ${${PKG}_I_STATIC_LIBRARIES})
      find_library(lib${lib} NAMES ${lib} PATHS
        ${${PKG}_I_STATIC_LIBRARY_DIRS} NO_DEFAULT_PATH)
      if (NOT lib${lib})
...
macro (libfind_check_pkgconfig PREFIX MOD)
  ...
  if (tmp_P_FOUND)
    ...
    foreach (lib ${tmp_P_STATIC_LIBRARIES})
      # First check in the library's own path, then in system path in case of
      # system wide libraries such as libm and libpthread.
      find_library("${PREFIX}_COMPONENT_${lib}" NAMES ${lib}
        PATHS ${tmp_P_STATIC_LIBRARY_DIRS} NO_DEFAULT_PATH)
      if (NOT ${PREFIX}_COMPONENT_${lib})
        find_library("${PREFIX}_COMPONENT_${lib}" NAMES ${lib})
      endif()
...
#
# Internal function: _normalize_double_slash(<OUT> [in...])
#
# Replace double slash with single slash in paths, so that paths like
# "/home/test/install//include" works in cmake's find_library and find_path.
#
function (_normalize_double_slash OUT)
  set(tmp "")
  foreach (n ${ARGN})
    string(REPLACE "//" "/" n ${n})
    string(STRIP n ${n})
...
#
# libfind_check_components(PREFIX [NO_DEFAULT_PATH] [OPTIONAL]
#                          NAMES name [name...] PATHS path [path...])
#
...
# variables to assist libfind_process. If OPTIONAL is specified, the variables
# are set only when the component is found.
#
macro (libfind_check_components PREFIX)
  cmake_parse_arguments(LFCC "NO_DEFAULT_PATH;OPTIONAL"
                        "" "NAMES;PATHS" ${ARGN})
  set(lfcc_args "")
  if (LFCC_NO_DEFAULT_PATH)
    set(lfcc_args NO_DEFAULT_PATH)
  endif()
  _normalize_double_slash(LFCC_PATHS ${LFCC_PATHS})
  foreach (c ${LFCC_NAMES})
    ...
    # Add to package's libs only when required
    if ((NOT LFCC_OPTIONAL) OR ${PREFIX}_COMPONENT_${c})
